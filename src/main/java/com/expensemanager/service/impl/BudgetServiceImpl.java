...
public class BudgetServiceImpl implements BudgetService {
    ...
    public BudgetLimit setLimit(BudgetDTO dto, User user) {
        ...
        LocalDate end   = dto.getEndDate()   != null ? dto.getEndDate()   : start.plusMonths(1);

        int month = start.getMonthValue();
        int year  = start.getYear();

        // For date-range budgets (individuals), create a new record each time
        // (same category can have multiple non-overlapping periods)
        ...
        limit.setYear(year);
        limit.setStartDate(start);
        limit.setEndDate(end);
        limit.setAlertSent(false);
        limit.setCurrentSpend(BigDecimal.ZERO);
    ...
    @Override
    @Transactional(readOnly = true)
    public List<BudgetLimit> getActiveLimitsForIndividual(User user) {
        return budgetLimitRepository.findActiveByUserDateRange(user, LocalDate.now());
    }
